...
AssumeRole() {
  ...
  if [ -z "$D_ROLE_EXTERNAL_ID" ]; then
    eval "$(aws sts assume-role-with-web-identity --role-arn "${D_ROLE_ARN}" --role-session-name "" --web-identity-token "${CIRCLE_OIDC_TOKEN}" --duration-seconds "${D_DURATION_SECONDS}" | jq -r '.Credentials | "export AWS_ACCESS_KEY_ID=\(.AccessKeyId)\nexport AWS_SECRET_ACCESS_KEY=\(.SecretAccessKey)\nexport AWS_SESSION_TOKEN=\(.SessionToken)\n"')"
  else
    eval "$(aws sts assume-role-with-web-identity --external-id "${D_ROLE_EXTERNAL_ID}" --role-arn "${D_ROLE_ARN}" --role-session-name "${D_SERVICE_NAME}" --web-identity-token "${CIRCLE_OIDC_TOKEN}" --duration-seconds "${D_DURATION_SECONDS}" | jq -r '.Credentials | "export AWS_ACCESS_KEY_ID=\(.AccessKeyId)\nexport AWS_SECRET_ACCESS_KEY=\(.SecretAccessKey)\nexport AWS_SESSION_TOKEN=\(.SessionToken)\n"')"
  fi
}
